...
export const Login: React.FC = () => {
  ...
  const [mode, setMode] = useState<'SELECT' | 'CASHIER' | 'CUSTOMER' | 'ADMIN' | 'BRANCH_MANAGER' | 'HOSPITALITY' | 'DEPARTMENT_STAFF' | 'ORDER_AGGREGATOR' | 'CALL_CENTER'>('SELECT');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    else if (mode === 'CALL_CENTER') login(name, 'CALL_CENTER', '', selectedBranch);
      ...
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6 text-slate-100" dir="rtl">
        <div className="max-w-7xl w-full">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <button onClick={() => setMode('CALL_CENTER')} className="group bg-slate-900 p-8 rounded-[3.5rem] border-2 border-white/5 hover:border-red-600 transition-all shadow-2xl flex flex-col items-center gap-6">
              ...
              <div className="text-center"><h3 className="text-xl font-black text-white">مركز الاتصالات</h3><p className="text-[10px] text-slate-500 mt-2 font-black uppercase tracking-widest">خدمة العملاء</p></div>
            </button>
    ...
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4 text-slate-100" dir="rtl">
      <div className="max-w-md w-full p-10 space-y-10 bg-slate-900 rounded-[4rem] border border-white/5 shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {(mode === 'BRANCH_MANAGER' || mode === 'DEPARTMENT_STAFF' || mode === 'ORDER_AGGREGATOR' || mode === 'CALL_CENTER') && (
            <div className="space-y-2">
